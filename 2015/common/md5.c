#include "md5.h"

#include <stdint.h>
#include <stdio.h>
#include <string.h>

// From https://en.wikipedia.org/wiki/MD5

// s specifies the per-round shift amounts
static uint32_t s[64] = {
    7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22,
    5, 9, 14, 20, 5, 9, 14, 20, 5, 9, 14, 20, 5, 9, 14, 20,
    4, 11, 16, 23, 4, 11, 16, 23, 4, 11, 16, 23, 4, 11, 16, 23,
    6, 10, 15, 21, 6, 10, 15, 21, 6, 10, 15, 21, 6, 10, 15, 21,
};

static uint32_t K[64] = {
    0xd76aa478, 0xe8c7b756, 0x242070db, 0xc1bdceee,
    0xf57c0faf, 0x4787c62a, 0xa8304613, 0xfd469501,
    0x698098d8, 0x8b44f7af, 0xffff5bb1, 0x895cd7be,
    0x6b901122, 0xfd987193, 0xa679438e, 0x49b40821,
    0xf61e2562, 0xc040b340, 0x265e5a51, 0xe9b6c7aa,
    0xd62f105d, 0x02441453, 0xd8a1e681, 0xe7d3fbc8,
    0x21e1cde6, 0xc33707d6, 0xf4d50d87, 0x455a14ed,
    0xa9e3e905, 0xfcefa3f8, 0x676f02d9, 0x8d2a4c8a,
    0xfffa3942, 0x8771f681, 0x6d9d6122, 0xfde5380c,
    0xa4beea44, 0x4bdecfa9, 0xf6bb4b60, 0xbebfbc70,
    0x289b7ec6, 0xeaa127fa, 0xd4ef3085, 0x04881d05,
    0xd9d4d039, 0xe6db99e5, 0x1fa27cf8, 0xc4ac5665,
    0xf4292244, 0x432aff97, 0xab9423a7, 0xfc93a039,
    0x655b59c3, 0x8f0ccc92, 0xffeff47d, 0x85845dd1,
    0x6fa87e4f, 0xfe2ce6e0, 0xa3014314, 0x4e0811a1,
    0xf7537e82, 0xbd3af235, 0x2ad7d2bb, 0xeb86d391,
};

typedef union {
    MD5Digest digest;
    struct {
        uint32_t a;
        uint32_t b;
        uint32_t c;
        uint32_t d;
    };
} MD5State;

MD5Digest md5(char *msg, size_t len) {
    MD5State state = {
        .a = 0x67452301,
        .b = 0xefcdab89,
        .c = 0x98badcfe,
        .d = 0x10325476,
    };
    size_t spilloverLen = len % 64;
    size_t chunkCount = (len - spilloverLen) / 64;
    char lastChunks[128] = {};
    memcpy(lastChunks, msg + 64 * chunkCount, spilloverLen);
    lastChunks[spilloverLen] = (char)0x80;
    size_t lastChunkCount = spilloverLen < 56 ? 1 : 2;
    uint64_t bitLength = len * 8;
    memcpy(lastChunks + (64 * lastChunkCount - 8), &bitLength, 8);

    // Process the message in successive 512-bit chunks:
    for (size_t chunk = 0; chunk < chunkCount + lastChunkCount; chunk++) {
        char *M;
        if (chunk < chunkCount) {
            M = msg + 64 * chunk;
        } else {
            M = lastChunks + 64 * (chunk - chunkCount);
        }

        uint32_t A = state.a;
        uint32_t B = state.b;
        uint32_t C = state.c;
        uint32_t D = state.d;
        for (int i = 0; i < 64; i++) {
            uint32_t F, g;
            if (i < 16) {
                F = (B & C) | (~B & D);
                g = i;
            } else if (i < 32) {
                F = (D & B) | (~D & C);
                g = (5 * i + 1) % 16;
            } else if (i < 48) {
                F = B ^ C ^ D;
                g = (3 * i + 5) % 16;
            } else {
                F = C ^ (B | ~D);
                g = (7 * i) % 16;
            }
            int32_t word;
            memcpy(&word, &M[4 * g], 4);
            F = F + A + K[i] + word;
            A = D;
            D = C;
            C = B;
            B = B + (F << s[i] | F >> (32 - s[i]));
        }
        state.a += A;
        state.b += B;
        state.c += C;
        state.d += D;
    }
    return state.digest;
}
