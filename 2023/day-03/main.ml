...
type pchar = Digit of int | Symbol of char | Blank
...
let symbols (pred : pchar -> bool) : pchar list list -> PartFlag.t list list =
  List.mapi @@ fun x ->
  List.mapi @@ fun y -> PartFlag.of_pchar pred x y
...
let blur2d : PartFlag.t list list -> PartFlag.t list list = transpose % List.map blur % transpose % List.map blur
...
let rec split_while_map (f : 'a -> 'b option) : 'a list -> 'b list * 'a list = function
  ...
  | x :: xs -> (
      match f x with
      | Some y ->
          let left, right = split_while_map f xs in
          (y :: left, right)
      | None -> ([], x :: xs))
...
let rec extract_numbers : (PartFlag.t * pchar) list -> (PartFlag.loc * int) list = function
  ...
  | (_, Digit _) :: _ as l -> (
      ...
      match List.filter_map fst digits with
...
let () =
  time @@ fun () ->
  solve_file "input-ex0.txt" @@ Some (4361, 467835);
  solve_file "input-real0.txt" @@ Some (540212, 87605697);
  ()
