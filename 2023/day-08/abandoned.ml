...
module MachineF (Mat : BoolMatType) : sig
  ...
  let create (lines : string list) : t =
    ...
    let (left_edges, right_edges) = List.split @@ flip List.map parsed @@ fun (from, (left, right)) ->
      ((index from, index left), (index from, index right))
    in
  ...
  let stream_ends m (dirs : string) : int -> int Stream.t =
    ...
    let rec recur dirs tick here =
      ...
      | None ->
        ...
        result
    in recur (Stream.cycle (List.of_seq (String.to_seq dirs))) 0
...
module Machine = struct
  ...
  let create (lines : string list) start_sufix end_suffix : t =
    ...
    let suffixed_indices s = List.concat %
      List.mapi (fun i (name, _) -> if String.ends_with ~suffix:s name then [i] else []) in
  ...
  let rec run m i dirs here =
    if i > 0 && IS.mem here m.end_states then (i, here, dirs) else
