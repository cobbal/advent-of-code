...
module Stream = struct
  ...
  let cycle (l : 'a list) : 'a t =
    let rec help : 'a list -> 'a t = function [] -> help l | x :: xs -> Cell (lazy (x, help xs)) in
    help l

  let rec take = function 0 -> fun _ -> [] | i -> fun (Cell (lazy (head, tail))) -> head :: take (i - 1) tail
end

let map_starts : 'a SM.t -> SS.t = SS.filter (String.ends_with ~suffix:"A") % SS.of_iter % SM.keys
let map_ends : 'a SM.t -> SS.t = SS.filter (String.ends_with ~suffix:"Z") % SS.of_iter % SM.keys
...
let follow map suffix =
  let rec follow i dirs here =
    if String.ends_with ~suffix here then i
    else
      let dir, dirs = Stream.uncons dirs in
      match SM.get here map with
      | None -> -1
      | Some (left, right) ->
