...
module SeedMap = struct
  ...
  let rec apply (map : t) (i : int) : int =
    match map with [] -> i | seg :: _ when IntRangeSet.mem i seg.source -> i + seg.offset | _ :: map -> apply map i

  let rec apply_set (map : t) (r : IntRangeSet.t) : IntRangeSet.t =
    match map with
    | [] -> r
    | seg :: rest ->
  ...
  let of_para : string list list -> t =
    let of_line : int list -> segment = function
      | [ map_to; from; count ] -> { source = IntRangeSet.range from (from + count - 1); offset = map_to - from }
      | _ -> fail "parse error"
    in
    List.map (of_line % List.map int_of_string) % List.tl
end
...
let solve1 (input : string list) : int =
  ...
  | [ "seeds:" :: seed_strings ] :: map_strings ->
      ...
      let seeds =
        List.fold_left IntRangeSet.union IntRangeSet.empty
        @@ List.map mk_range @@ pairs @@ List.map int_of_string seed_strings
      in
      (*     print_endline @@ [%show: IntRangeSet.t] seeds; *)
      let loc_of_seeds seeds = List.fold_left (fun r map -> SeedMap.apply_set map r) seeds maps in
...
let () =
  time @@ fun () ->
  solve_file "input-ex0.txt" @@ Some (35, 46);
  solve_file "input-real0.txt" @@ Some (525792406, 79004094);
  ()
