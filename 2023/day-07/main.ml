...
let parse_hand card_order (input : string) = match words input with
| [hand; bid] -> (List.map (String.index card_order) (List.of_seq (String.to_seq hand)), int_of_string bid)
| _ -> fail "parse error"

let unparse_hand card_order = 
  String.of_list % List.map (String.get card_order)
...
let solve_file (filename : string) expected =
  ...
  let result = (
    solve @@ List.map (parse_hand card_order0) input,
    solve @@ List.map (parse_hand card_order1) input
  ) in
  print_string @@ Fmt.str "%s: %s" filename @@ [%show: int * int] result;
  check_results ~expected:expected ~actual:result

let () = solve_file "input-ex0.txt" @@ Some (6440, 5905)
let () = solve_file "input-real0.txt" @@ Some (248422077, 249817836)
