...
let parse (line : string) : string * int array =
  ...
  | [ springs; num_str ] -> (springs, Array.of_list @@ List.map int_of_string (String.split_on_char ',' num_str))
  | _ -> fail "bad parse"

let solve_line springs runs =
  let springs_len = String.length springs in
  let runs_len = Array.length runs in
  let isolve : int * int -> int =
    IIMemo.memo_fix (fun isolve (i, j) ->
        if i = springs_len && j = runs_len then 1
        else if i >= springs_len then 0
        else
          let eat_run () =
            if j >= runs_len then 0
            else
              let len = runs.(j) in
              if i + len > springs_len then 0
              else if not (String.for_all (String.contains "#?") (String.sub springs i len)) then 0
              else
                let more_left = i + len < springs_len in
                if more_left && Char.(String.get springs (i + len) = '#') then 0
                else isolve (min (i + len + 1) springs_len, j + 1)
          in
          let pass () = isolve (i + 1, j) in
          let res =
            match String.get springs i with
            | '.' -> pass ()
            | '#' -> eat_run ()
            | '?' -> pass () + eat_run ()
            | _ -> fail "bad char"
...
let unfold (springs, runs) = (String.concat "?" (List.init 5 (const springs)), Array.concat (List.init 5 (const runs)))
