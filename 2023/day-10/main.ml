...
let bend (start : dir) (tile : char) : dir option = match tile, start with
| '|', N | 'J', E | 'L', W -> Some N
| 'F', N | '-', E | 'L', S -> Some E
| '7', E | '|', S | 'F', W -> Some S
| '7', N | 'J', S | '-', W -> Some W
...
if y < 0 || Array.length g <= y then '.' else
let row = g.(y) in
if x < 0 || Array.length row <= x then '.' else
row.(x)
...
let start (g : grid) : loc =
  Option.get_exn_or "No start found" @@
      flip Array.find_mapi g @@ fun y row ->
        match Array.find_index Char.(fun c -> c = 'S') row with
        | None -> None
        | Some x -> Some (y, x)
...
let walk (g : grid) : (loc * dir) list =
    let rec recur trail l d =
      ...
      if Char.(peek g l' = 'S') then Some (List.rev trail') else
      match bend d (peek g l') with
      | None -> None
      | Some d' -> recur trail' l' d'
    in
    List.hd (List.filter_map (recur [] (start g)) [N; E; S; W])

let parse : string list -> grid =
  Array.of_list % List.map String.to_array
...
let verts (g : grid) : grid =
  ...
  flip List.iter (walk g) @@ begin fun ((y, x), d) ->
  ...
  flip List.iter (List.init (height - 1) id) @@ begin fun y ->
      flip List.iter (List.init width id) @@ begin fun x ->
        if Char.(mat.(2 * y).(2 * x) = ' ') then
          mat.(2 * y + 1).(2 * x) <- '.'
      end
  end;
  mat
...
let count_in : grid -> int =
  ...
  in Array.fold_left (+) 0 % Array.map (recur false 0 % Array.to_list)
