...
let range lo hi : t = range_canon { lo; hi }
...
let range_union x y =
  ...
  else if range_mem (pred y.lo) x || range_mem (succ y.hi) y || range_mem x.lo y || range_mem y.lo x then
    range (min x.lo y.lo) (max x.hi y.hi)
  else [ x; y ]
...
let canon : t -> t =
  let rec help = function
    ...
    | [ x ] -> range_canon x
    | x :: y :: rest -> (
        match range_union x y with [ xy ] -> help (xy :: rest) | _ -> range_canon x @ help (y :: rest))
  in
  help % List.sort (fun x y -> x.lo - y.lo)
