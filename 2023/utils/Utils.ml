...
let rec repeatedly i f x = match i with 0 -> x | i -> repeatedly (i - 1) f (f x)
let rec tails : 'a list -> 'a list list = function [] -> [] | _ :: xs as l -> l :: tails xs
...
let rec zip2d (xs : 'a list list) (ys : 'b list list) : ('a * 'b) list list =
  match (xs, ys) with [], _ -> [] | _, [] -> [] | x :: xs, y :: ys -> List.combine x y :: zip2d xs ys
...
let check_results ~expected ~actual =
  match expected with
  | Some expectation ->
      if Stdlib.(expectation = actual) then print_endline " \u{2705}"
      else (
        print_endline " \u{274c}";
        fail "Wrong answer")
  | None -> print_newline ()
...
let rec split_by (pred : 'a -> bool) : 'a list -> 'a list list = function
  ...
  | x :: xs ->
      let first, rest = List.hd_tl (split_by pred xs) in
      (x :: first) :: rest
...
let rec pairs = function [] -> [] | [ x ] -> fail "odd length list to pairs" | x :: y :: rest -> (x, y) :: pairs rest
